import { Activity, Itinerary, PlannerInput } from './types';
...
function fallbackItinerary(input: PlannerInput): Itinerary {
  ...
  const budgetPerDay = input.budget === 'Budget' ? 35 : input.budget === 'Mid-range' ? 90 : 220;
...
export async function generateItinerary(input: PlannerInput): Promise<Itinerary> {
  const apiKey =
    (process.env as Record<string, string | undefined>).EXPO_PUBLIC_GEMINI_API_KEY ||
    (process.env as Record<string, string | undefined>).GEMINI_API_KEY;
  const days = diffDaysInclusive(input.startDate, input.endDate);

  if (!apiKey) return fallbackItinerary(input);
