...
export async function getWeatherByCoords(lat: number, lng: number): Promise<WeatherResponse | null> {
  try {
    const url =
    ...
    const data = (await res.json()) as { current?: OpenMeteoCurrent; daily?: OpenMeteoDaily };
