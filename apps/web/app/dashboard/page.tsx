...
function Dashboard() {
  ...
  const trending = destinations.slice(2, 6);
      ...
      <section className="relative overflow-hidden rounded-3xl p-8 md:p-12 bg-gradient-to-br from-brand-500 via-brand-600 to-emerald-800 text-white">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="mt-3 text-white/80 max-w-lg">
            Tell our AI your vibe and get a day-by-day plan, optimised routes,
            and live weather — in seconds.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Link
              href="/planner"
              className="btn bg-white text-brand-700 hover:bg-white/90 font-semibold"
            >
        ...
        <div className="flex items-end justify-between mb-4">
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Loaded from Firestore destinations
            </p>
          </div>
        </div>
        <DestinationGrid
...
function DestinationGrid({
  ...
  destinations: ReturnType<typeof useDestinations>["destinations"];
  loading: boolean;
  error: string | null;
  compact?: boolean;
}) {
  ...
  if (error) {
    return <div className="card p-5 text-sm text-red-600">{error}</div>;
  }
  ...
  return (
    <div
      className={`grid grid-cols-2 ${compact ? "md:grid-cols-4" : "md:grid-cols-3 lg:grid-cols-4"} gap-4`}
    >
      {destinations.map((d) => (
