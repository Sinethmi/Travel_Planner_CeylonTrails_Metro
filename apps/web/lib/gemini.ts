...
import { Destination, haversineKm } from './srilanka';

const GEMINI_URL =
  'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent';

function buildPrompt(input: PlannerInput, days: number, destinations: Destination[]): string {
  const knownLocations = destinations.map(
...
function optimiseRoute(activities: Activity[]): Activity[] {
  ...
  const ordered: Activity[] = [activities[0]!];
  const pool = activities.slice(1);
  while (pool.length) {
...
function fallbackItinerary(input: PlannerInput, destinations: Destination[]): Itinerary {
  ...
  const picks = pool.length ? pool : destinations;
  const budgetPerDay =
...
export async function generateItinerary(input: PlannerInput, destinations: Destination[]): Promise<Itinerary> {
  const apiKey = process.env.GEMINI_API_KEY;
  const days = diffDaysInclusive(input.startDate, input.endDate);

  if (!apiKey) {
    return fallbackItinerary(input, destinations);
  }
  ...
  try {
    const res = await fetch(`${GEMINI_URL}?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        contents: [{ role: 'user', parts: [{ text: buildPrompt(input, days, destinations) }] }],
        generationConfig: {
          temperature: 0.85,
          topP: 0.95,
    ...
    if (!text) return fallbackItinerary(input, destinations);
